...
from alien_game_2p.alienProductExchange import AlienProductExchange
...
class Player(BasePlayer):
    ...
    def live_selection(self, data):
        ...
        if 'chat' in data:
            ...
            #
            # if trial == 1:
            #     self.session.vars[self.group.id_in_subsession]['chat'][self.session.vars[self.group.id_in_subsession]["trial_number"][self.round_number - 1] - 1] = data["chat"]
            # else:
            #     round_chat = data['chat']
            #     for round, prev_chat in self.session.vars[self.group.id_in_subsession]['chat'][self.session.vars[self.group.id_in_subsession]["trial_number"][self.round_number - 1] - 1].items():
            #         if round != trial:
            #             previous_chat_messages = [c + "'§" for c in prev_chat.split("'§")][:-1]
            #
            #             for old_text in previous_chat_messages:
            #                 round_chat = round_chat.replace(old_text, "")
            #
            #     self.session.vars[self.group.id_in_subsession]['chat'][self.session.vars[self.group.id_in_subsession]["trial_number"][self.round_number - 1] - 1] += round_chat
            #
            # self.group.Chat = str(self.session.vars[self.group.id_in_subsession]['chat'])
            return
        ...
        index = 0 if result['playerId'] == 1 else 1
        ...
        return {0:
            dict(
                ...
                p1_payoff=self.session.vars[g_id]["p1_payoffs"][self.round_number - 1][-1],
                p2_payoff=self.session.vars[g_id]["p2_payoffs"][self.round_number - 1][-1],
