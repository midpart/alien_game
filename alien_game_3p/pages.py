...
from alien_game_3p.alienProductExchange import AlienProductExchange
...
class AlienMarket(Page):
    ...
    def is_displayed(self):
        ...
        return self.player.session.vars[self.player.group.id_in_subsession]['trial_number'][self.round_number - 1] <= self.session.vars['max_trials']

    def vars_for_template(self):
        g_id = self.player.group.id_in_subsession
        selections = self.player.session.vars[g_id]["selections"][self.round_number - 1]
        ...
        channel = f"R{self.player.round_number}T{self.player.session.vars[g_id]['trial_number'][self.round_number - 1] - 1}"

        payoffs = []

        for p in self.group.get_players():
            payoffs.append(self.session.vars[g_id][f"p{p.id_in_group}_payoffs"][self.round_number - 1])

        return dict(trial_number=self.player.session.vars[g_id]["trial_number"][self.round_number - 1] - 1,
                    ...
                    joint_payoffs=self.player.session.vars[g_id]["selections_payoff"][self.round_number - 1],
                    #p1_payoffs=self.player.session.vars[g_id]["p1_payoffs"][self.round_number - 1],
                    #p2_payoffs=self.player.session.vars[g_id]["p2_payoffs"][self.round_number - 1],
                    payoffs=payoffs,
